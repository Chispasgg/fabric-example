...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm hyperledger/fabric-tools:2.4.7 peer version | sed -ne 's/^ Version: //p')
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    CA_DOCKER_IMAGE_VERSION=$(docker run --rm hyperledger/fabric-ca:1.5.5 fabric-ca-client version | sed -ne 's/ Version: //p' | head -1)
